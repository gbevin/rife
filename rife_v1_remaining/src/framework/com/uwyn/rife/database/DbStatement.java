 ...
 * $Id: DbStatement.java 3946 2008-05-04 09:12:20Z gbevin $
 */
 ...
 * @see #getResultSet()
 * @see com.uwyn.rife.database.DbConnection#createStatement
...
public class DbStatement implements Cloneable
{
	 ...
	 * @exception DatabaseException if a database access error occurs, or the
	...
	throws DatabaseException
	{
		try
		{
			mConnection.releaseStatement(this);
		...
		{
			throw new StatementCloseErrorException(mConnection.getDatasource(), e);
		}
	}
	...
	throws DatabaseException
	{
		try
		{
			...
			boolean result = mStatement.execute(sql);
			outputTrace(start, sql);
			
			setResultset(mStatement.getResultSet());
		...
		{
			handleException();
			throw new ExecutionErrorException(sql, mConnection.getDatasource(), e);
		}
	}
	...
	throws DatabaseException
	{
		try
		{
			...
			long start = startTrace();
			boolean result = mStatement.execute(sql, autoGeneratedKeys);
			outputTrace(start, sql);
	...
	throws DatabaseException
	{
		try
		{
			...
			long start = startTrace();
			boolean result = mStatement.execute(sql, columnIndexes);
			outputTrace(start, sql);
	...
	throws DatabaseException
	{
		try
		{
			...
			long start = startTrace();
			boolean result = mStatement.execute(sql, columnNames);
			outputTrace(start, sql);
	...
	throws DatabaseException
	{
		...
		{
			handleException();
			throw new BatchExecutionErrorException(mConnection.getDatasource(), e);
		}
	}
	...
	throws DatabaseException
	{
		try
		{
			return wrapWithDbResultSet(mStatement.getGeneratedKeys());
		}
		catch (SQLException e)
		{
			handleException();
	...
	throws DatabaseException
	{
		try
		{
			...
			boolean result = mStatement.getMoreResults();
			setResultset(mStatement.getResultSet());
			return result;
		}
		catch (SQLException e)
	...
	throws DatabaseException
	{
		try
		{
			...
			boolean result = mStatement.getMoreResults(current);
			setResultset(mStatement.getResultSet());
			return result;
		}
		catch (SQLException e)
	 ...
	 * @exception DatabaseException if a database access error occurs or this
	...
	throws DatabaseException
	{
		try
		{
			mStatement.setQueryTimeout(max);
		}
		catch (SQLException e)
		{
			handleException();
	 ...
	 *
	 * @exception DatabaseException when a database access error occurs or the
	...
	private DbResultSet wrapWithDbResultSet(ResultSet resultSet) throws DatabaseException
	{
		...
		try
		{
			if (JavaSpecificationUtils.isAtLeastJdk16())
			{
				...
				{
					throw new DatabaseException("You're running a JDK that's compatible with version 1.6, this requires a RIFE distribution that is compiled specifically for it. Please upgrade the RIFE jar you're using to one that supports JDK 1.6.", e);
				}
			}
			else
			{
				...
				{
					throw new DatabaseException("You're not running a JDK that's compatible with version 1.6, this requires a RIFE distribution that is compiled specifically for a previous version of the JDK. Please replace the RIFE jar you're using with one that is built for your JDK version.", e);
				}
			}
			
			Constructor constructor = resulset_class.getDeclaredConstructor(DbStatement.class, ResultSet.class);
			return (DbResultSet)constructor.newInstance(this, resultSet);
		}
