 ...
 * $Id: TestConstrainedBean.java 3918 2008-04-14 17:35:35Z gbevin $
 */
...
public class TestConstrainedBean extends TestCase
{
	public TestConstrainedBean(String name)
	{
		super(name);
	}
	...
	public void testUniques()
	{
		...
		assertSame(bean, bean.uniques(new ArrayList<String[]> () {{ add(first_unique); add(second_unique); add(third_unique); }} ));
		assertTrue(bean.hasUniques());
		assertEquals(3, bean.getUniques().size());
		assertSame(first_unique, bean.getUniques().get(0));
		assertSame(second_unique, bean.getUniques().get(1));
