 ...
 * $Id: TestPurgingMixedAuthenticated.java 3918 2008-04-14 17:35:35Z gbevin $
 */
...
public class TestPurgingMixedAuthenticated extends TestsuiteDatabaseAuthenticated
{
	private Datasource	mDatasource = null;
	
	public TestPurgingMixedAuthenticated(String datasourceName, int siteType, String name)
	{
		...
		mProperties.put(SimpleSessionManagerFactory.PROPERTYNAME_MANAGER_ID, "purging_mixed_pgsql");
		mProperties.put(SimpleSessionManagerFactory.PROPERTYNAME_MANAGER_CLASS, "MemorySessions");
		mProperties.put(SessionManagerFactoryFactory.PROPERTYNAME_FACTORY_CLASS, SimpleSessionManagerFactory.class.getName());
		mProperties.put(RememberManagerFactoryFactory.PROPERTYNAME_FACTORY_CLASS, DatabaseRememberFactory.class.getName());
	}
	
	private SessionManager getSessionManager()
	{
		return SessionManagerFactoryFactory.getManager(mProperties);
	}
	...
	public void testPurgingMixedAuthenticatedBasic()
	throws Exception
	{
		...
		request = new GetMethodWebRequest("http://localhost:8181/authentication/purging/mixed/basic");
		...
		assertEquals(1, getSessionManager().countSessions());
		...
		assertEquals(2, getSessionManager().countSessions());
		...
		assertEquals(1, getSessionManager().countSessions());
		getSessionManager().eraseAllSessions();
	}

	public void testPurgingMixedAuthenticatedEncrypted()
	throws Exception
	{
		setupSite("site/authentication_database.xml");
		...
		request = new GetMethodWebRequest("http://localhost:8181/authentication/purging/mixed/encrypted");
		response = conversation.getResponse(request);
		form = response.getForms()[0];
	...
	public void testPurgingMixedAuthenticatedRemember()
	throws Exception
	{
		setupSite("site/authentication_database.xml");
		...
		request = new GetMethodWebRequest("http://localhost:8181/authentication/purging/mixed/remember");
		response = conversation.getResponse(request);
