 ...
 * $Id: TestXml2ElementInfo.java 3928 2008-04-22 16:25:18Z gbevin $
 */
...
public class TestXml2ElementInfo extends TestCase
{
	public TestXml2ElementInfo(String name)
	{
		super(name);
	}
	...
	public void testInstantiation()
	{
		...
		ElementInfoBuilder	elementinfo_builder = site_builder.enterElement("manual://test");
	...
	{
		SiteBuilder			site_builder = new SiteBuilder("test", ResourceFinderClasspath.getInstance());
		ElementInfoBuilder	elementinfo_builder = site_builder.enterElement("xml:xml/test_xml2elementinfo1.xml");
		...
		assertEquals(elementinfo.getPropertyNames().size(), 7+Rep.getProperties().size());
		...
		assertTrue(elementinfo.getProperty("property2") instanceof Template);
		assertEquals(((Template)elementinfo.getProperty("property2")).getContent(), TemplateFactory.ENGINEHTML.get("noblocks_in").getContent());
		assertEquals("the value 3", elementinfo.getProperty("property3"));
		assertSame(Datasources.getRepInstance().getDatasource("unittestspgsql"), elementinfo.getProperty("property4"));
		assertEquals("unittests\n		pgsql", elementinfo.getProperty("property5"));
		try
		{
			elementinfo.getProperty("property6");
			fail("exception wasn't thrown");
		}
		catch (PropertyValueRetrievalException e)
		{
			assertTrue(e.getCause() instanceof ParticipantUnknownException);
			assertEquals("doesntexist", ((ParticipantUnknownException)e.getCause()).getName());
		}
		try
		{
			elementinfo.getProperty("property7");
			fail("exception wasn't thrown");
		}
		catch (PropertyValueRetrievalException e)
		{
			assertTrue(e.getCause() instanceof TemplateFactoryUnknownException);
			assertEquals("unknowntype", ((TemplateFactoryUnknownException)e.getCause()).getType());
		}
	...
	public void testFlawedSubmissionParamRegexpname()
	{
		SiteBuilder			site_builder = new SiteBuilder("test", ResourceFinderClasspath.getInstance());
		ElementInfoBuilder	elementinfo_builder = site_builder.enterElement("element/flawed/submission_param_regexpname.xml");
	...
	public void testFlawedSubmissionParamRegexpdefaults()
	{
		SiteBuilder			site_builder = new SiteBuilder("test", ResourceFinderClasspath.getInstance());
		ElementInfoBuilder	elementinfo_builder = site_builder.enterElement("element/flawed/submission_param_regexpdefaults.xml");
