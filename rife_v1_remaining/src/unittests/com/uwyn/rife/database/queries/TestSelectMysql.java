 ...
 * $Id: TestSelectMysql.java 3918 2008-04-14 17:35:35Z gbevin $
 */
...
{
	public TestSelectMysql(String name)
	{
		super(name);
	}
	...
	public void testHintMysql()
	{
		...
		query
			.hint("SQL_CACHE")
			.from("tablename");
		assertEquals(query.getSql(), "SELECT SQL_CACHE * FROM tablename");
		assertTrue(execute(query));
	}
	...
	{
		Select query = new Select(mMysql, BeanImplConstrained.class);
		query.from("tablename");
		assertEquals(query.getSql(), "SELECT * FROM tablename ORDER BY propertyString ASC, propertyInt DESC");
		assertTrue(execute(query));

		query = new Select(mMysql, BeanImplConstrained.class);
		query.from("tablename")
	...
	{
		Select unionquery1 = new Select(mMysql);
		unionquery1
			.from("table2")
		...
		Select unionquery2 = new Select(mMysql);
		unionquery2
			.from("table2")
