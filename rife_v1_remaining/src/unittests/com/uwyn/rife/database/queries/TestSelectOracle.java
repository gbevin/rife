 ...
 * $Id: TestSelectOracle.java 3918 2008-04-14 17:35:35Z gbevin $
 */
...
{
	public TestSelectOracle(String name)
	{
		super(name);
	}
	...
	{
		Select query = new Select(mOracle);
		assertNotNull(query);
		try
		{
			query.getSql();
	...
	{
		Select query = new Select(mOracle, BeanImplConstrained.class);
		query.from("tablename");
		assertEquals(query.getSql(), "SELECT * FROM tablename ORDER BY propertyString ASC, propertyInt DESC");
		assertTrue(execute(query));

		query = new Select(mOracle, BeanImplConstrained.class);
		query.from("tablename")
	...
	public void testSubselectParamsOracle()
	{
		...
		Select tablequery = new Select(mOracle);
		tablequery
			.from("table2")
		...
		Select unionquery1 = new Select(mOracle);
		unionquery1
			.from("table2")
		...
		Select unionquery2 = new Select(mOracle);
		unionquery2
			.from("table2")
		...
		query
			...
			.whereOr("tablename.propertyFloat", ">", new Select(mOracle)
				...
				.whereParameter("propertyLong", "!="))
			.from("tablename", new Select(mOracle)
				.from("tablename"))
