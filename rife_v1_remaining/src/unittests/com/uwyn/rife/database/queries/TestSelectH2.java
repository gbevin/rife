 ...
 * $Id: TestSelectH2.java 3920 2008-04-14 19:58:18Z gbevin $
 */
...
{
	public TestSelectH2(String name)
	{
		super(name);
	}
	...
	{
		Select query = new Select(mH2, BeanImplConstrained.class);
		query.from("tablename");
		assertEquals(query.getSql(), "SELECT * FROM tablename ORDER BY propertyString ASC, propertyInt DESC");
		assertTrue(execute(query));

		query = new Select(mH2, BeanImplConstrained.class);
		query.from("tablename")
	...
	{
		Select fieldquery = new Select(mH2);
		fieldquery
			.from("table2")
			.field("propertyLong")
			.whereParameter("propertyInt", ">")
			.limit(1);
		Select tablequery = new Select(mH2);
		tablequery
			.from("table2")
		...
		Select unionquery1 = new Select(mH2);
		unionquery1
			.from("table2")
		...
		Select unionquery2 = new Select(mH2);
		unionquery2
			.from("table2")
