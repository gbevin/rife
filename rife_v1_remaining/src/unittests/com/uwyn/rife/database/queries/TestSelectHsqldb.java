 ...
 * $Id: TestSelectHsqldb.java 3918 2008-04-14 17:35:35Z gbevin $
 */
...
{
	public TestSelectHsqldb(String name)
	{
		super(name);
	}
	...
	{
		Select query = new Select(mHsqldb, BeanImplConstrained.class);
		query.from("tablename");
		assertEquals(query.getSql(), "SELECT * FROM tablename ORDER BY propertyString ASC, propertyInt DESC");
		assertTrue(execute(query));

		query = new Select(mHsqldb, BeanImplConstrained.class);
		query.from("tablename")
