 ...
 * $Id: TestCreateTableHsqldb.java 3918 2008-04-14 17:35:35Z gbevin $
 */
...
{
	public TestCreateTableHsqldb(String name)
	{
		super(name);
	}
	...
	public void testDefaultFunctionHsqldb()
	{
		...
		assertEquals(query.getSql(), "CREATE TABLE tablename1 (dateobject DATE DEFAULT CURRENT_TIMESTAMP)");
		execute(query);
	}
	...
	public void testCustomAttributeHsqldb()
	{
		...
		assertEquals(query.getSql(), "CREATE TABLE tablename1 (intColumn INTEGER IDENTITY)");
		execute(query);
	}
	...
	public void testTemporaryHsqldb()
	{
		...
		assertEquals(query.getSql(), "CREATE TEMP TABLE tablename (boolColumn BOOLEAN)");
		execute(query);
	}
	...
	public void testForeignKeyViolationsSingleHsqldb()
	{
		...
		query.table("tablename")
			.column("intColumn", int.class)
			.defaultValue("intColumn", 0)
			.foreignKey("foreigntable", "intColumn", "foreignIntColumn", CreateTable.SETDEFAULT, null);
		assertEquals(query.getSql(), "CREATE TABLE tablename (intColumn INTEGER DEFAULT 0, FOREIGN KEY (intColumn) REFERENCES foreigntable (foreignIntColumn) ON UPDATE SET DEFAULT)");
		execute(query);
		query.clear();
		...
		assertEquals(query.getSql(), "CREATE TABLE tablename (intColumn INTEGER DEFAULT 0, FOREIGN KEY (intColumn) REFERENCES foreigntable (foreignIntColumn) ON DELETE SET DEFAULT)");
		execute(query);
		query.clear();
	...
	public void testForeignKeyMultipleViolationsHsqldb()
	{
		...
		assertEquals(query.getSql(), "CREATE TABLE tablename (intColumn INTEGER DEFAULT 0, stringColumn VARCHAR(50) DEFAULT 'def', FOREIGN KEY (intColumn, stringColumn) REFERENCES foreigntable (foreignIntColumn, foreignStringColumn) ON UPDATE RESTRICT ON DELETE SET DEFAULT)");
		execute(query);
	}
	...
	public void testCloneHsqldb()
	{
		...
		query.table("tablename")
			...
			.precision("propertyTime", 9)
			.precision("propertyTimestamp", 30, 2)
			.nullable("propertyString", CreateTable.NOTNULL)
			.defaultValue("propertyString", "stringDefault")
			.defaultValue("propertyInt", -1)
			.defaultFunction("propertyDate", "CURRENT_TIMESTAMP")
			.customAttribute("propertyByte", "DEFAULT 0")
			.temporary(true)
