 ...
 * $Id: TestSelectPgsql.java 3918 2008-04-14 17:35:35Z gbevin $
 */
...
{
	public TestSelectPgsql(String name)
	{
		super(name);
	}
	...
	public void testBeanConstrainedPgsql()
	{
		Select query = new Select(mPgsql, BeanImplConstrained.class);
		query.from("tablename");
		assertEquals(query.getSql(), "SELECT * FROM tablename ORDER BY propertyString ASC, propertyInt DESC");
		assertTrue(execute(query));

		query = new Select(mPgsql, BeanImplConstrained.class);
		query.from("tablename")
